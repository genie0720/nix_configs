...
{
  home.file.".config/hypr/hyprland/scripts/ai/show-loaded-ollama-models.sh".text = ''
    #!/bin/bash
    ...
    compare_running_models_and_modelfiles() { 
        ...
        OLDIFS=$IFS
        for ((i=0; i<${#model_name_paths[@]}; i++)); do
  ...
  home.activation.makeShowModelsExecutable = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    chmod +x ${config.home.homeDirectory}/.config/hypr/hyprland/scripts/ai/show-loaded-ollama-models.sh
  '';
}
